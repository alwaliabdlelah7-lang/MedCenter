...
export default function DoctorManagement() {
    ...
    const loadData = async () => {
      try {
        ...
        if (savedFields) {
          ...
          setDynamicFields(
            allFields.filter((f) => f.entity === "doctor" && f.isActive),
          );
        }
      } catch (error) {
  ...
  const stats = {
    ...
    avgFee: doctors.length
      ? Math.round(
          doctors.reduce((acc, d) => acc + d.consultationFee, 0) /
            doctors.length,
        )
      : 0,
  };
    ...
    <div className="space-y-8 lg:p-4 text-right">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h2 className="text-3xl font-black text-white tracking-tight">
            إدارة الكادر الطبي والخبراء
          </h2>
          <p className="text-sm text-sky-400/70 border-r-4 border-sky-600 pr-4 mt-2 font-bold italic">
            تتبع ساعات العمل، الرسوم، والبيانات المهنية للفريق الطبي
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative group">
            <Search
              className="absolute right-3 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-sky-400 transition-colors"
              size={18}
            />
            <input
              type="text"
          ...
          <button
            onClick={() =>
              printReport("تقرير الكادر الطبي", "doctors-report-list")
            }
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <SummaryCard
          icon={DollarSign}
          label="متوسط الكشفية"
          value={`${stats.avgFee.toLocaleString()} ر.ي`}
          color="emerald"
        />
      </div>

      <div
        id="doctors-report-list"
            ...
            >
              ...
              <div className="flex justify-between items-start relative z-10">
                <div className="flex gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-sky-400/70 font-black italic mt-1">
                      {doctor.specialization}
                    </p>
                    <div className="flex items-center gap-4 mt-3">
                      <span className="text-[10px] bg-white/5 px-2 py-1 rounded-lg text-slate-500 font-mono font-bold">
                        ID: {doctor.id}
                      </span>
              ...
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-10 relative z-10">
                <StatItem
                  icon={DollarSign}
                  label="رسم المعاينة"
                  value={`${doctor.consultationFee.toLocaleString()} ر.ي`}
                  color="emerald"
                />
                <StatItem
                  icon={Clock}
                  label="ساعات العمل"
                  value={`${doctor.workingHours.start} - ${doctor.workingHours.end}`}
                  color="amber"
                />
                <StatItem
                  icon={Calendar}
                  label="أيام العمل"
                  value={`${doctor.workingDays.length} أيام`}
                  color="sky"
                />
                <StatItem
                  icon={ActivityIcon}
              ...
              <div className="mt-8 flex items-center justify-between relative z-10 pt-6 border-t border-white/5">
                <div className="flex gap-2">
                  {doctor.workingDays.map((day) => (
                    <span
                      key={day}
                      className="w-8 h-8 rounded-xl glass bg-white/5 border border-white/10 flex items-center justify-center text-[10px] font-black text-slate-500 uppercase hover:bg-sky-500 hover:text-white transition-colors tracking-tighter"
                    >
                      {day.substring(0, 3)}
                    </span>
                  ))}
              ...
              <form className="space-y-6" onSubmit={handleAddOrUpdate}>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-500 uppercase italic block">
                      القسم
                    </label>
                    <select
                      name="departmentId"
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                  ...
                  <InputGroup
                    name="percentage"
                    label="نسبة الطبيب (%)"
                    defaultValue={editingDoctor?.percentage}
                    type="number"
                    icon={ActivityIcon}
                    required
                  />
                  <InputGroup
                    name="followupFee"
                    label="رسوم المراجعة"
                    defaultValue={editingDoctor?.followupFee}
                    type="number"
                    icon={DollarSign}
                  />
                  <InputGroup
                    name="returnDays"
                    label="فترة المراجعة (يوم)"
                    defaultValue={editingDoctor?.returnDays}
                    type="number"
                    icon={Calendar}
                  />
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-500 uppercase italic block">
                    ملاحظات مهنية / سيرة ذاتية مختصره
                  </label>
                  <textarea
                    name="notes"
                    defaultValue={editingDoctor?.notes}
                    className="w-full px-5 py-4 glass bg-white/5 border border-white/10 rounded-2xl text-white outline-none font-bold min-h-[100px]"
                ...
                {dynamicFields.length > 0 && (
                  <div className="pt-6 border-t border-white/5 space-y-6">
                    <h4 className="text-sm font-black text-sky-400 flex items-center gap-2 italic uppercase">
                      <Tag size={18} /> بيانات إضافية مخصصة
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {dynamicFields.map((field) => (
                        <div key={field.id} className="space-y-2 text-right">
                          ...
                          <input
                            type={field.type}
                            defaultValue={
                              editingDoctor?.customFields?.[field.name]
                            }
                ...
                <div className="pt-8 flex gap-4">
                  <button
                    type="submit"
                    className="flex-1 py-5 bg-sky-600 text-white rounded-3xl font-black shadow-2xl shadow-sky-600/30 hover:bg-sky-500 transition-all uppercase tracking-[4px]"
                  >
                  ...
                  <button
                    type="button"
                    onClick={() => setShowAddModal(false)}
                    className="flex-1 py-5 glass bg-white/5 text-slate-500 rounded-3xl font-black hover:bg-white/10 transition-all uppercase tracking-[4px]"
                  >
                    إلغاء
                  </button>
                </div>
...
function SummaryCard({ icon: Icon, label, value, color }: any) {
  ...
  return (
    <div className="glass p-6 rounded-[32px] border border-white/5 flex items-center gap-5 group hover:bg-white/5 transition-all">
      <div
        className={cn(
          "w-14 h-14 rounded-2xl flex items-center justify-center border shadow-inner group-hover:scale-110 transition-transform",
          colors[color],
        )}
      >
        <Icon size={28} />
  ...
  return (
    <div className="space-y-2 text-right">
      ...
      <div className="relative group">
        {Icon && (
          <Icon
            className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-sky-500 transition-colors"
            size={18}
          />
        )}
        <input
